...
echo "================================================================"
echo "          ImmortalWrt H5000M 固件工程 GitHub 推送脚本"
echo "================================================================"
echo ""
...
if [ -n "$(git status --porcelain)" ]; then
    ...
    if [ -z "$COMMIT_MSG" ]; then
        if [ -t 0 ]; then
            ...
            COMMIT_MSG="${USER_MSG:-feat: update firmware config, packages and scripts}"
        else
            COMMIT_MSG="feat: update firmware config, packages and scripts"
        fi
    fi
    echo "正在暂存并提交代码..."
    git add .
